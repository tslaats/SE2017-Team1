...
public class ActivityDrawingFactory {
    ...
    private void drawHeader(Graphics2D g2d, String text, int x , int y){
      ...
      x = x + (Constants.EVENT_WIDTH - metrics.stringWidth(text)) / 2;
    ...
    private void drawBody(Graphics2D g2d, String text, int x, int y){
      ...
      for (String line : toDraw.split("\n")){
          int tempX = x + (Constants.EVENT_WIDTH - metrics.stringWidth(line)) / 2;
          //draw each line
    ...
    private void drawActivityOutline(Graphics2D g2d, int upperLeftX, int upperLeftY, int lowerRightX){
      ...
      g2d.fillRect(upperLeftX, upperLeftY, Constants.EVENT_WIDTH, Constants.EVENT_HEIGHT);
      g2d.setColor(Color.BLACK);
      g2d.drawRect(upperLeftX, upperLeftY, Constants.EVENT_WIDTH, Constants.EVENT_HEIGHT);
    ...
    public void drawActivity(Graphics2D g2d, Activity activity) {
        ...
        int upperLeftX = center.x - Constants.EVENT_WIDTH / 2;
        int upperLeftY = center.y - Constants.EVENT_HEIGHT / 2;
        int lowerRightX = center.x + Constants.EVENT_WIDTH /2;
