...
public class RelationDrawingFactory {
    ...
    private void drawVaguelyZigZaggyLine(Graphics2D g2d, Point start, Point end, String dir) {
      ...
      Point p4 = end;
      if(dir == "R") {
        p2 = new Point(start.x + Constants.EVENT_PADDING, start.y);
        p3 = new Point(end.x - Constants.EVENT_PADDING, end.y);
      }else if(dir == "L") {
        p2 = new Point(start.x - Constants.EVENT_PADDING, start.y);
        p3 = new Point(end.x + Constants.EVENT_PADDING, end.y);
      }else if(dir == "U") {
        p2 = new Point(start.x, start.y - Constants.EVENT_PADDING);
        p3 = new Point(end.x, end.y + Constants.EVENT_PADDING);
      }else if(dir == "D") {
        p2 = new Point(start.x, start.y + Constants.EVENT_PADDING);
        p3 = new Point(end.x, end.y - Constants.EVENT_PADDING);
      }else{
    ...
    private void drawSelfLine(Graphics2D g2d, Point start, String dir) {
      ...
      if(dir == "R") {
        p2 = new Point(start.x + Constants.EVENT_PADDING, start.y);
        p3 = new Point(start.x + Constants.EVENT_PADDING, start.y + Constants.EVENT_PADDING/2);
        p4 = new Point(start.x, start.y + Constants.EVENT_PADDING/2);
      }else if(dir == "L") {
        p2 = new Point(start.x - Constants.EVENT_PADDING, start.y);
        p3 = new Point(start.x - Constants.EVENT_PADDING, start.y - Constants.EVENT_PADDING/2);
        p4 = new Point(start.x, start.y - Constants.EVENT_PADDING/2);
        ...
        p2 = new Point(start.x, start.y - Constants.EVENT_PADDING);
        p3 = new Point(start.x + Constants.EVENT_PADDING/2, start.y - Constants.EVENT_PADDING);
        p4 = new Point(start.x + Constants.EVENT_PADDING/2, start.y);
      }else if(dir == "D") {
        p2 = new Point(start.x, start.y + Constants.EVENT_PADDING);
        p3 = new Point(start.x + Constants.EVENT_PADDING/2, start.y + Constants.EVENT_PADDING);
        p4 = new Point(start.x + Constants.EVENT_PADDING/2, start.y);
    ...
    public void drawResponseRelation(Graphics2D g2d, Relation rel) {
      ...
      if(rel.getChild().equals(rel.getParent())) {
        ...
        if(dir == "L") {
          dir = "R";
          selfEnd = new Point(p1.x, p1.y - Constants.EVENT_PADDING/2);
        }else if(dir == "R") {
          dir = "L";
          selfEnd = new Point(p1.x, p1.y + Constants.EVENT_PADDING/2);
        }else if(dir == "D") {
          dir = "U";
          selfEnd = new Point(p1.x + Constants.EVENT_PADDING/2, p1.y);
        }else if(dir == "U") {
          dir = "D";
          selfEnd = new Point(p1.x + Constants.EVENT_PADDING/2, p1.y);
        }
  ...
  public Set<Relation> setOutRelations(Activity act, Set<Relation> allRelations) {
    ...
    Relation selfRel = new Relation(act, act, Type.Response);
    ...
    if(selfRelation) {
      ...
      if(minSurface == rightRels.size()) {
        leastBusySurface = "R";
        selfRel.setOut(new Point(act.getCenter().x + Constants.EVENT_WIDTH/2, act.getCenter().y - Constants.EVENT_HEIGHT/2 + Constants.TRIANGLE_HEIGHT/2));
        ...
        leastBusySurface = "T";
        selfRel.setOut(new Point(act.getCenter().x - Constants.EVENT_WIDTH/2 + Constants.TRIANGLE_HEIGHT/2, act.getCenter().y - Constants.EVENT_HEIGHT/2));
      }else if(minSurface == bottomRels.size()) {
        leastBusySurface = "B";
        selfRel.setOut(new Point(act.getCenter().x + Constants.TRIANGLE_HEIGHT/2, act.getCenter().y + Constants.EVENT_HEIGHT/2));
      }
    ...
    if(rightRels.size() > 0) {
      ...
      surfaceLength = Constants.EVENT_HEIGHT / 2;
      spacing = surfaceLength / numberOutgoing;
      if(leastBusySurface == "R") {
        offset = Constants.EVENT_PADDING + spacing;
      }
      rightRels = sortByChildY(rightRels);
      for(Relation rel : rightRels) {
        Point start = new Point(act.getCenter().x + Constants.EVENT_WIDTH/2, act.getCenter().y - Constants.EVENT_HEIGHT/2 + Constants.TRIANGLE_HEIGHT/2 + i*spacing + offset);
        rel.setOut(start);
    ...
    if(leftRels.size() > 0) {
      ...
      for(Relation rel : leftRels) {
        Point start = new Point(act.getCenter().x - Constants.EVENT_WIDTH/2, act.getCenter().y + Constants.TRIANGLE_HEIGHT/2 + i*spacing);
        rel.setOut(start);
        returnRelations.add(rel);
        i++;
      }
    ...
    if(topRels.size() > 0) {
      ...
      surfaceLength = Constants.EVENT_WIDTH / 2;
      spacing = surfaceLength / numberOutgoing;
      ...
      for(Relation rel : topRels) {
        Point start = new Point(act.getCenter().x - Constants.EVENT_WIDTH/2 + Constants.TRIANGLE_HEIGHT/2 + i*spacing + offset, act.getCenter().y - Constants.EVENT_HEIGHT/2);
        rel.setOut(start);
        returnRelations.add(rel);
        i++;
      }
    ...
    if(bottomRels.size() > 0) {
      ...
      for(Relation rel : bottomRels) {
        Point start = new Point(act.getCenter().x + Constants.TRIANGLE_HEIGHT/2 + i*spacing + offset, act.getCenter().y + Constants.EVENT_HEIGHT/2);
        rel.setOut(start);
        returnRelations.add(rel);
        i++;
      }
  ...
  public Set<Relation> setInRelations(Activity act, Set<Relation> allRelations) {
    ...
    if(rightRels.size() > 0) {
      ...
      rightRels = sortByChildY(rightRels);
      for(Relation rel : rightRels) {
        Point end = new Point(act.getCenter().x + Constants.EVENT_WIDTH/2, act.getCenter().y + Constants.TRIANGLE_HEIGHT/2 + i*spacing);
        rel.setIn(end);
        returnRelations.add(rel);
        i++;
      }
    ...
    if(leftRels.size() > 0) {
      ...
      leftRels = sortByChildY(leftRels);
      for(Relation rel : leftRels) {
        Point end = new Point(act.getCenter().x - Constants.EVENT_WIDTH/2, act.getCenter().y - Constants.EVENT_HEIGHT/2 + Constants.TRIANGLE_HEIGHT/2 + i*spacing);
        rel.setIn(end);
        returnRelations.add(rel);
        i++;
      }
    ...
    if(topRels.size() > 0) {
      ...
      topRels = sortByChildX(topRels);
      for(Relation rel : topRels) {
        Point end = new Point(act.getCenter().x + Constants.TRIANGLE_HEIGHT/2 + i*spacing, act.getCenter().y - Constants.EVENT_HEIGHT/2);
        rel.setIn(end);
        returnRelations.add(rel);
        i++;
      }
    ...
    if(bottomRels.size() > 0) {
      ...
      bottomRels = sortByChildX(bottomRels);
      for(Relation rel : bottomRels) {
        Point end = new Point(act.getCenter().x - Constants.EVENT_WIDTH/2 + Constants.TRIANGLE_HEIGHT/2 + i*spacing, act.getCenter().y + Constants.EVENT_HEIGHT/2);
        rel.setIn(end);
        returnRelations.add(rel);
        i++;
      }
